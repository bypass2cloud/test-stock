...
async function accessToken(appKey: string, appSecret: string, baseUrl: string) {
  ...
  if (!response.ok) throw new Error("인증 토큰을 발급하지 못했습니다.");
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const quotes = await Promise.all(symbols.map(async (symbol) => {
      ...
      if (!response.ok) throw new Error(`${symbol} 시세를 불러오지 못했습니다.`);
      ...
      return { code: symbol, name: output.hts_kor_isnm || quoteNames[symbol]?.name || symbol, price: Number(output.stck_prpr || 0).toLocaleString("ko-KR"), change: `${sign < 0 ? "−" : "+"}${Math.abs(rate).toFixed(2)}%`, color: quoteNames[symbol]?.color || "#2563eb" };
    }));
